"""Aggregates Washington-located listings from every taildraggers.com
...
ones whose location ends in ", WA".
...
Vermont/Virginia repos' approach, with the state filter swapped to "WA".
"""
...
PAGE_TITLE = "Taildragger Ads in Washington"
STATE_ABBR = "WA"
...
_WASHINGTON_LOCATION_RE = re.compile(r",\s*WA\s*$", re.IGNORECASE)


def _is_washington(location: str) -> bool:
    return bool(_WASHINGTON_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        washington_rows = [row for row in rows if _is_washington(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(washington_rows)} in Washington")
        all_listings.extend(washington_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Washington listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Washington listings: {len(listings)}")
