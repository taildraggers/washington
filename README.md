...
Daily aggregator of Washington-located taildragger classified listings,
...
[Virginia](https://github.com/taildraggers/virginia) repos' approach, with the
state filter swapped to "WA".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, WA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Washington"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/washington/`
...
```html
<iframe
  src="https://taildraggers.github.io/washington/"
  title="Taildragger Ads in Washington"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_WASHINGTON_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
